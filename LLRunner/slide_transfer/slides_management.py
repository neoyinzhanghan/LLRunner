...
def get_topview(wsi_name):
    ...
    assert os.path.exists(slide_path), f"Slide {wsi_name} not found in the tmp_slide_dir."

    slide = openslide.OpenSlide(slide_path)
    topview = slide.read_region((0, 0), topview_level, slide.level_dimensions[topview_level])
...
def get_level_0_mpp(wsi_name):
    ...
    assert openslide.PROPERTY_NAME_MPP_X in slide.properties, f"Slide {wsi_name} does not have the property {openslide.PROPERTY_NAME_MPP_X}."
...
def copy_slide_to_tmp(wsi_name, overwrite=False, overwrite_topview=False):
    ...
    if os.path.exists(slide_path):
        if overwrite:
            ...
            slide_metadata_row["slide_last_updated"] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            ...
            try:
                ...
                topview_path = os.path.join(tmp_slide_dir, "topview", wsi_name.replace(slide_metadata_row["wsi_extension"], ".jpg"))
