...
from LLRunner.slide_transfer.sshos import SSHOS, rsync_file_local
...
def copy_slide_to_tmp(wsi_name, overwrite=False, overwrite_topview=False):
    ...
    if os.path.exists(slide_path):
        if overwrite:
            ...
            rsync_file_local(
                file=wsi_name,
                source_dir=slide_source_dir,
                destination_dir=tmp_slide_dir,
            )
    ...
    # update_slide_metadata(metadata_row_dct=slide_metadata_row, overwrite=True) #TODO for concurrent processing, we are removing this line to prevent file lock inconsistencies
